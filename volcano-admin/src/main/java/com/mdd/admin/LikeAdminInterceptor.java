...
public class LikeAdminInterceptor implements HandlerInterceptor {
    ...
    @Resource
    IUserService iSystemAuthAdminService;
    ...
    public boolean preHandle(@NotNull HttpServletRequest request,
        ...
        if (!RedisUtils.hExists(AdminConfig.backstageManageKey, id)) {
            iSystemAuthAdminService.cacheAdminUserByUid(Long.valueOf(id.toString()));
        }
...
//        if (notAuthUri.contains(auths) || Integer.parseInt(id.toString()) == 1) {
        List<Long> roleIds = ArrayUtils.stringToListAsLong(userMap.get("roleIds"), ",");

        if (notAuthUri.contains(auths) || SpecialRoleEnum.SUPER_ADMIN.getCode() == roleIds.get(0)) {
            return HandlerInterceptor.super.preHandle(request, response, handler);
        }
